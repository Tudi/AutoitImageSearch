...
char answer[DEFAULT_STR_BUFFER_SIZE];
...
// (since it's called in so many places).
{
	...
	strncpy_s(aDst, DEFAULT_STR_BUFFER_SIZE, aSrc, aDstSize); // NOTE: In spite of its zero-filling, strncpy() benchmarks considerably faster than a custom loop, probably because it uses 32-bit memory operations vs. 8-bit.
	aDst[aDstSize] = '\0';
}
...
// Author: ImageSearch was created by Aurelian Maga.
{
	...
	char color_name[DEFAULT_STR_BUFFER_SIZE], *dp;
